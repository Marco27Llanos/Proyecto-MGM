...
class CtrlCarrito extends Controlador {
    public function agregar() {
        ...
        if (is_object($_SESSION['carrito'])){
            ...
            if(isset($_GET['url'])){
                switch($_GET['url']){
                    case 'detalles':
                        header("Location: ?ctrl=CtrlEquipo&accion=verDetalles&id=".$_GET['id']);
                        exit();
                        break;
                    case 'carrito':
                        $this->mostrar();
                        ...
                        break;
                    default:
                        header("Location: ?ctrl=CtrlEquipo&accion=getCatalogo&id=".$_GET['id']);
                        exit();
                }
            }else{
                header("Location: ?ctrl=CtrlEquipo&accion=getCatalogo&id=".$_GET['id']);
    ...
    public function mostrar() {
        ...
        if (!isset($_SESSION['carrito'])){
            ...
            # Recuperar Equipo según CARRITO
            $obj = new Equipo();
            
            $data=$obj->getEquiposCarrito();
            if (is_null($data))
                $miCarrito=Vista::mostrar('carrito/vacio.php','',true);
            else
